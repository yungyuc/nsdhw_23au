...
private:
    ...
    {
        return row * m_ncol + col;  // row + col * m_nrow;
    }
...
PYBIND11_MODULE(_matrix, m) {   // module name
    ...
    pybind11::class_<Matrix>(m, "Matrix")
        ...
        .def_property_readonly("array",
            [](Matrix& mat) -> pybind11::array_t<double> {
                return pybind11::array_t<double>(pybind11::memoryview::from_buffer(
                    mat.buffer(),                                   // buffer pointer
                    { mat.nrow(), mat.ncol() },                     // shape (rows, cols)
                    { sizeof(double) * mat.ncol(), sizeof(double) } // strides in bytes
            ));
        })
